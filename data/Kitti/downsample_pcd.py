import os
import os.path as osp
import open3d as o3d
import numpy as np
import glob
from tqdm import tqdm


def main():
    if not osp.exists('downsampled'):
        os.makedirs('downsampled')
    
    for i in range(11):
        seq_id = '{:02d}'.format(i)
        seq_dir = osp.join('downsampled', seq_id)
        if not osp.exists(seq_dir):
            os.makedirs(seq_dir)
    
    for i in range(11):
        seq_id = '{:02d}'.format(i)
        path = '/data/test04/datasets/01_kitti/data_odometry_velodyne/dataset/'
        file_names = glob.glob(osp.join(path,'sequences', seq_id, 'velodyne', '*.bin'))
        for file_name in tqdm(file_names):
            frame = file_name.split('/')[-1][:-4]
            new_file_name = osp.join('downsampled', seq_id, frame + '.npy')
            points = np.fromfile(file_name, dtype=np.float32).reshape(-1, 4)
            points = points[:, :3]
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(points)
            pcd = pcd.voxel_down_sample(0.3)
            points = np.array(pcd.points).astype(np.float32)
            np.save(new_file_name, points)


if __name__ == '__main__':
    main()
