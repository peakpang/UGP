...
def make_open3d_axis(axis_vector=None, origin=None, scale=1.0):
    ...
    if axis_vector is None:
        axis_vector = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=float)
...
def make_open3d_axes(axis_vectors=None, origin=None, scale=1.0):
    ...
    if axis_vectors is None:
        axis_vectors = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=float)
...
def make_open3d_corr_lines(ref_corr_points, src_corr_points, label, color=None):
    ...
    if label == "pos":
        ...
        corr_lines.paint_uniform_color(color)
    else:
...
def registration_with_ransac_from_correspondences(
    ...
    result = o3d.pipelines.registration.registration_ransac_based_on_correspondence(
        ...
        estimation_method=o3d.pipelines.registration.TransformationEstimationPointToPoint(False),
        checkers=[
            o3d.pipelines.registration.CorrespondenceCheckerBasedOnDistance(0.4),
        ],
        ransac_n=ransac_n,
        criteria=o3d.pipelines.registration.RANSACConvergenceCriteria(num_iterations, 0.999),
    )
