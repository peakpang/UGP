...
from ugp.utils.summary_board import SummaryBoard
from ugp.utils.timer import Timer
from ugp.utils.torch import all_reduce_tensors, release_cuda, initialize
from ugp.engine.logger import Logger
...
def inject_default_parser(parser=None):
    ...
    parser.add_argument('--distance', type=int, default=10, help='distance threshold for metadata')
    return parser
...
class BaseTrainer(abc.ABC):
    ...
    def register_model(self, model):
        ...
        if self.distributed:
            ...
            model = nn.parallel.DistributedDataParallel(model, device_ids=[local_rank],find_unused_parameters=True, output_device=local_rank)
        self.model = model
