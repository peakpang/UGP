...
class SuperPointMatching(nn.Module):
    ...
    def forward(self, ref_feats, src_feats, ref_masks=None, src_masks=None):
        ...
        matching_scores = torch.exp(-pairwise_distance(ref_feats, src_feats, normalized=True)) # N X M
...
class SuperPointMatchingWithOverlap(nn.Module):
    def __init__(self, num_correspondences, dual_normalization=True):
        super(SuperPointMatchingWithOverlap, self).__init__()
        self.num_correspondences = num_correspondences
        self.dual_normalization = dual_normalization

    def forward(self, ref_feats, src_feats, ref_overlap_confidence, src_overlap_confidence, ref_masks=None, src_masks=None):
        r"""Extract superpoint correspondences.
        ...
        if src_masks is None:
            src_masks = torch.ones(size=(src_feats.shape[0],), dtype=torch.bool).cuda()
        ref_raw_indices = torch.nonzero(ref_masks, as_tuple=True)[0]
        src_raw_indices = torch.nonzero(src_masks, as_tuple=True)[0]
        
        ref_nooverlap_indices = torch.nonzero(ref_overlap_confidence < 0.2,as_tuple=True)[0]
        src_nooverlap_indices = torch.nonzero(src_overlap_confidence < 0.2,as_tuple=True)[0]
        ref_masks[ref_nooverlap_indices] = False
        src_masks[src_nooverlap_indices] = False
        
        # remove empty patch
        ref_indices = torch.nonzero(ref_masks, as_tuple=True)[0]
        src_indices = torch.nonzero(src_masks, as_tuple=True)[0]
        if ref_indices.numel() < 3:
            ref_indices = ref_raw_indices
        if src_indices.numel() < 3:
            src_indices = src_raw_indices
