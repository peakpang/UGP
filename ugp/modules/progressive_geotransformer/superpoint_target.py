...
class SuperPointTargetGeneratorWithOverlap(nn.Module):
    def __init__(self, num_targets, overlap_threshold):
        super(SuperPointTargetGeneratorWithOverlap, self).__init__()
        self.num_targets = num_targets
        self.overlap_threshold = overlap_threshold

    @torch.no_grad()
    def forward(self, gt_corr_indices, gt_corr_overlaps,ref_overlap_gt, src_overlap_gt,ref_size,src_size):
        r"""Generate ground truth superpoint (patch) correspondences.
        ...
        """
        gt_corr_masks = torch.gt(gt_corr_overlaps, self.overlap_threshold)
        ref_masks = torch.ones(size=(ref_size,), dtype=torch.bool).cuda()
        src_masks = torch.ones(size=(src_size,), dtype=torch.bool).cuda()
        gt_corr_overlaps = gt_corr_overlaps[gt_corr_masks]
        gt_corr_indices = gt_corr_indices[gt_corr_masks]
